...
def get_longitude_latitude(address):
	...
	try:
		r = requests.get('http://www.datasciencetoolkit.org/maps/api/geocode/json?sensor=false&address={}'.format(
			address
		))
...
def fetch_geo():
	...
	addresses_df = df['street_address'].drop_duplicates().to_frame('street_address')
	...
	# We create a dataframe with 2 new columns, by merging with the series returned by the lambda functio
